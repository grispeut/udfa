...
class LDGFLHead(AnchorHead):
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
                    bbox_targets, stride, soft_targets, featmap, featmap_kd, num_total_samples, return_only_fm=False):
    ...
    def forward_train(self,
        ...
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas, img, x)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas, img, x)

        if return_only_fm:
            losses_cls = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore, img_clean=img_clean, fm_backbone=fm_backbone, return_only_fm=True)
            return neck_mask_batch, bb_mask_batch, losses_cls, outs[0]


        losses = self.loss(*loss_inputs, gt_bboxes_ignore=gt_bboxes_ignore, img_clean=img_clean, fm_backbone=fm_backbone)


        if return_neck_mask:
            return losses, neck_mask_batch, bb_mask_batch

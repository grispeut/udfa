...
@PIPELINES.register_module()
class ImageCorrupt(object):
    def __init__(self, severity_max=2):
        super().__init__()
    ...
    def __call__(self, results):
        ...
        severity = random.randint(1, self.severity_max)
        if idx not in [-1] :
            img = corrupt(img, corruption_number=idx, severity=severity)
            results['img'] = img
        return results
